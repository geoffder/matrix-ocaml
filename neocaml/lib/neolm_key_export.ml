...
let sha512_pbkdf2 ~password ~salt ~count =
  Pbkdf.pbkdf2 ~prf:`SHA512 ~password ~salt ~count ~dk_len |> Fn.flip Cstruct.split @@ 32
...
let encrypt ?(count = 100000) pass data =
  ...
  in
  let payload =
    Cstruct.concat [ Cstruct.of_string "\001"; salt; ctr; count_cs; encrypted ]
  in
...
let decrypt pass payload =
  let%bind decoded =
    Base64.decode ~pad:false payload
    |> Result.map_error ~f:(fun _ -> `InvalidBase64)
    >>| Cstruct.of_string
  in
  Result.ok_if_true
    (Cstruct.get_uint8 decoded 0 = 1)
    ~error:(`Protocol "Unsupported export format version")
  >>= fun () ->
  let password = Cstruct.of_string pass in
  let salt = Cstruct.sub decoded 1 16 in
  let ctr = Cstruct.sub decoded 17 16 in
  let count =
    Cstruct.sub decoded 33 4 |> Cstruct.to_bytes |> Neolm_utils.bigend_bytes_to_int
  in
  ...
  let aes_key, hmac_key = sha512_pbkdf2 ~password ~salt ~count in
  let hmac =
    Cstruct.sub decoded 0 (Cstruct.len decoded - hmac_len)
    |> Nocrypto.Hash.SHA256.hmac ~key:hmac_key
  in
  Result.ok_if_true
    (Cstruct.equal expected_hmac hmac)
    ~error:(`Protocol "HMAC check failed for encrypted payload.")
  >>| fun () ->
  let open Nocrypto.Cipher_block.AES in
  CTR.decrypt encrypted ~key:(CTR.of_secret aes_key) ~ctr |> Cstruct.to_string

let encrypt_and_save ?count outfile pass data =
  let%map encrypted = encrypt ?count pass data in
