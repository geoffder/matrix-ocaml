...
module ToDeviceList = struct
  ...
  let of_yojson j =
    let collect acc j =
      ...
      | _    -> acc
    in
    let%map.Result l = U.member "events" j |> list_of_yojson in
    List.fold ~init:[] ~f:collect (List.rev l) |> fun events -> { events }
end
...
module KeysQuery = struct
  ...
  type t =
    { failures : Yojson.Safe.t StringMap.t
    ; device_keys : device_keys StringMap.t StringMap.t
    }
  [@@deriving of_yojson]
end

module UpdateDevice = Empty (struct
  let fail = "User has no device with given ID."
end)

module DeleteDevices = Empty (struct
  let fail = "Additional authentication is required."
end)
...
module SetPresence = Empty (struct
  let fail = "Could not set presence."
end)
...
module SetDisplayName = Empty (struct
  let fail = "Could not set display name."
end)
...
module SetAvatar = Empty (struct
  let fail = "Could not set avatar url."
end)
...
module RoomInvite = Empty (struct
  let fail = "Invite failed."
end)

module RoomLeave = Empty (struct
  let fail = "Failed to leave room."
end)

module RoomForget = Empty (struct
  let fail = "Failed to forget room."
end)

module RoomKick = Empty (struct
  let fail = "Failed to kick user from room."
end)

module RoomBan = Empty (struct
  let fail = "Failed to ban user from room."
end)

module RoomUnban = Empty (struct
  let fail = "Failed to unban user from room."
end)
...
module RoomTyping = Empty (struct
  let fail = "Failed to send typing notification."
end)

module UpdateReceiptMarker = Empty (struct
  let fail = "Failed to send receipt."
end)

module RoomReadMarkers = Empty (struct
  let fail = "Failed to update read markers."
end)
...
module KeysClaim = struct
  ...
  let failures_of_yojson j =
    try U.keys j |> Result.return with
    | _ -> Result.fail "Expected `Assoc of server contact failures."
...
module ToDevice = Empty (struct
  let fail = "Failed to send to-device message."
end)
...
module Sync = struct
  module Timeline = struct
    type t =
      { events : Event.Timeline.t list option [@default None]
  ...
  module UnreadNotificationCounts = struct
    type t =
      { highlight_count : int option [@default None]
  ...
  module JoinedRooms = struct
    type info =
      ...
      ; msc2654_unread_count : int option
            [@key "org.matrix.msc2654.unread_count"] [@default None]
      }
  ...
  module Rooms = struct
    type t =
      { invite : InvitedRooms.t option [@default None]
  ...
  module DeviceLists = struct
    type t =
      { changed : string list option [@default None]
