...
module DeviceStore = struct
  ...
  let device_from_sender_key t user_id sender_key =
    ...
    |> Option.bind
         ~f:(Sequence.find ~f:(fun d -> String.equal sender_key (Device.curve25519 d)))
  ...
  let add t (d : Device.t) =
    ...
    | Some u ->
      ...
      in
      Map.set t ~key:d.user_id ~data
    | None   ->
      Map.add_exn t ~key:d.user_id ~data:(Map.of_alist_exn (module String) [ d.id, d ])
      |> Result.return
end
